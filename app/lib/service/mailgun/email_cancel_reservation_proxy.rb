module Service
  module Mailgun
    class EmailCancelReservationProxy
      ...
      def send!
        body=@email_generator.generate_cancel_reservation!
        
        @api_adapter
        .set_to(@listener_song_request.listener.email)
        .set_from(AppConfig::ServiceSettings.mailgun_email_address)
        .set_tag(AppConfig::ServiceSettings.cancel_reservation_email_tag)
        .set_subject(AppConfig::ServiceSettings.cancel_reservation_email_subject)
        .set_body(body)
        .send!
      end
    end
