...
describe Service::Mailgun::EmailCancelReservationProxy do
  ...
  let(:proxy){Service::Mailgun::EmailSongApprovalProxy.new(reservation)}
  let(:mailgun_email_address){'mailgun_email@mailgun.com'}
  let(:subject){'song approved'}  
  let(:tag){'song approved'}
  let(:body){'<html><body>song approved</body></html>'}
  ...
  before do
    ...
    allow(AppConfig::ServiceSettings)
    .to receive(:song_approve_email_subject)
    .and_return(subject)
    
    allow(AppConfig::ServiceSettings)
    .to receive(:song_approve_email_tag)
    .and_return(tag)
    
    allow(proxy.email_generator)
    .to receive(:generate_song_approval!)
    .and_return(body)
